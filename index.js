...
function findMinMax(numbers) {
  ...
  if (!Array.isArray(numbers)) {
    console.error("Помилка: вхідне значення має бути масивом");
    return null;
  }
...
function compareAndRound(num1, num2) {
  ...
  const roundedMaxNum = Math.round(maxNum);
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  if (!Array.isArray(products)) {
    console.error("Помилка: аргумент має бути масивом.");
    return null;
  }

  if (typeof maxPrice !== "number" || isNaN(maxPrice)) {
    console.error("Помилка: maxPrice має бути числом.");
    return null;
  }
  ...
  const totalPrice = products.reduce((accumulator, product) => {
    if (typeof product.price !== "number" || isNaN(product.price)) {
      console.error(
        `Помилка: price для продукту ${product.title} має бути числом.`
      );
      return accumulator;
    }
